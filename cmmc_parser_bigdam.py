#!/usr/bin/env python3
"""
CMMC SSP Parser - CSV to HTML/DOCX Converter (Updated Structure)
Converts CMMC control documentation from CSV format to HTML and DOCX
"""
...
class CMMCParser:
    ...
    def generate_html_for_control(self, row):
        ...
        # Evidence_Strings (with underscore - exact field name from CSV)
        if 'Evidence_Strings' in row and not pd.isna(row['Evidence_Strings']) and row['Evidence_Strings'] != '':
            html += """
                <div class="evidence-strings">
                    <h4>Evidence Strings</h4>
                    <ul>
            """
            for evidence in self.parse_delimited_content(row['Evidence_Strings']):
                html += f"<li>{evidence}</li>"
            html += """
                    </ul>
                </div>
    ...
    def generate_docx_for_control(self, doc, control):
        ...
        # Evidence_Strings (with underscore - exact field name from CSV)
        if 'Evidence_Strings' in control and not pd.isna(control['Evidence_Strings']) and control['Evidence_Strings'] != '':
            doc.add_heading('Evidence Strings', 3)
            for evidence in self.parse_delimited_content(control['Evidence_Strings']):
                if evidence:
                    p = doc.add_paragraph(style='List Bullet')
                    p.add_run(evidence)
...
"""
    ...
    def run(self, filter_controls=None, filter_families=None, filter_range=None):
        ...
        try:
            # Load CSV (supports multiple delimiters)
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='CMMC SSP Parser - Convert CSV to HTML/DOCX')
